...
import shutil
from syndot.utils.file_actions import copy, change_parent_owner, change_child_owner, remove
...
class TestChangeChildOwner:
    ...
    def test_function(self, source_path):
        ...
        for path in [source_path, destination_path]:
            ...
            create_file_or_directory(path = os.path.join(path, 'child_directory'), is_file = False)
            create_file_or_directory(path = os.path.join(path, 'child_directory', 'sub_child_file'), is_file = True)
            create_file_or_directory(path = os.path.join(path, 'child_file'), is_file = True)
        ...
        for source_target, destination_target in zip(source_content, destination_content):
            source_target_stats = os.stat(os.path.join(source_path, source_target))
            destination_target_stats = os.stat(os.path.join(destination_path, destination_target))
            assert source_target_stats.st_uid == destination_target_stats.st_uid
            assert source_target_stats.st_gid == destination_target_stats.st_gid
