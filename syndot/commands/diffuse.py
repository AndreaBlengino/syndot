    ...
    if targets_list:
        ...
        if n_targets > 1:
            ...
            for system_target_path, settings_target_path in targets_list.items():
                print(f"{system_target_path} <-- {settings_target_path}")
            ...
            if os.path.isfile(list(targets_list.keys())[0]):
                ...
                print(f"{list(targets_list.keys())[0]} <-- {list(targets_list.values())[0]}")
        ...
        if proceed:
            for i, (system_target_path, settings_target_path) in enumerate(targets_list.items(), 1):
                print(f"Diffusing {settings_target_path} to {system_target_path}")
