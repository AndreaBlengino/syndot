...
diffuse_targets.add_argument(
    ...
    required=False,
    dest='all',
    help='Select all dotfiles in the <MAP_FILE>, mandatory if <TARGET_PATH> '
...
diffuse_parser.add_argument(
    ...
    required=False,
    dest='exact',
    help='Search for an exact match for the <TARGET_PATH> in the map file. '
         'If not provided, search for all targets which paths begin with '
         '<TARGET_PATH> in the map file')
diffuse_parser.add_argument(
    '-h', '--help',
    action='help',
    default=SUPPRESS,
