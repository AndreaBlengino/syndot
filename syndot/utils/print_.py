...
def print_action(
        ...
        settings_target_path: str) -> None:
    if action_type == 'link':
        action = 'Linking'
        preposition = 'to'
        print(f"{action} {Color.link(system_target_path)} "
              f"{preposition} {Color.settings(settings_target_path)}")
    elif action_type == 'unlink':
        action = 'Unlinking'
        preposition = 'from'
        print(f"{action} {Color.link(system_target_path)} {preposition} "
              f"{Color.settings(settings_target_path)}")
    elif action_type == 'diffuse':
        action = 'Diffusing'
        preposition = 'to'
        print(f"{action} {Color.settings(settings_target_path)} {preposition} "
...
def print_relationship(
        ...
        symbol: str) -> None:
    print(f"{Color.link(system_target_path)} {Color.symbol(symbol)} "
          f"{Color.settings(settings_target_path)}")
